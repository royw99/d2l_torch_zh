...
def reorg_train_valid(data_dir, labels, valid_ratio, work_dir):
    ...
    for train_file in os.listdir(os.path.join(data_dir,'train')):
        ...
        copyfile(fname,os.path.join(work_dir,'train_valid_test','train_valid',label)) 
        if label not in label_count or label_count[label] < n_valid_per_label:                           
            copyfile(fname,os.path.join(work_dir,'train_valid_test','valid',label))
            label_count[label] = label_count.get(label,0) + 1
        else:
            copyfile(fname,os.path.join(work_dir,'train_valid_test','train',label))  
    return n_valid_per_label

def reorg_test(data_dir,work_dir):
    for test_file in os.listdir(os.path.join(data_dir,'test')):
        copyfile(os.path.join(data_dir,'test',test_file),
                os.path.join(work_dir,'train_valid_test','test','unknown')) # unknown为 test文件夹里面的一个文件夹   
